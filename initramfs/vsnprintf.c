...
static void vsnprintf_write(struct vsnprintf_sink *sink,
			const char *data, size_t size)
{ sink->write(sink, data, size); }

static void vsnprintf_putchar(struct vsnprintf_sink *sink, int c)
{
	const char buf = c;
...
static void vsnprintf_repeat(struct vsnprintf_sink *sink, int c, size_t count)
{
	for (size_t i = 0; i != count; ++i)
		vsnprintf_putchar(sink, c);
}

static void vsnprintf_puts_nonewline(struct vsnprintf_sink *sink,
			const char *str)
{
...
static void format_number(struct vsnprintf_sink *sink, uintmax_t value,
			const struct format_spec *spec)
{
	...
	vsnprintf_repeat(sink, ' ', padding);
	vsnprintf_puts_nonewline(sink, sign);
	vsnprintf_puts_nonewline(sink, prefix);
	vsnprintf_puts_nonewline(sink, num);
}
...
void __vsnprintf(struct vsnprintf_sink *sink, const char *fmt, va_list args)
{
	...
	while (*fmt) {
		...
		switch (spec.type) {
		case FT_NONE: {
			vsnprintf_write(sink, save, read);
			break;
		}
		...
		case FT_CHAR: {
			...
			vsnprintf_repeat(sink, ' ', MAX(spec.width, 0));
			vsnprintf_putchar(sink, c);
			break;
		}
		...
		case FT_STR: {
			...
			vsnprintf_repeat(sink, ' ', MAX(spec.width - len, 0));
			vsnprintf_puts_nonewline(sink, toprint);
			break;
		}

		case FT_PERCENT:
			vsnprintf_putchar(sink, '%');
			break;
